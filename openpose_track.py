...
class OpenPose_Track_Video(object):
    ...
    def check_openpose_lib(self):
        ...
        # sys.path.append('/home/javis/VR_Stadium_Openpose/openpose/build/python')
    ...
    def set_params(self):
        ...
        params["output_resolution"] = self.config.output_resolution
        ...
        params["alpha_pose"] = self.config.alpha_pose
        params["scale_gap"] = self.config.scale_gap
        params["scale_number"] = self.config.scale_number
        params["render_threshold"] =self.config.render_threshold
        # # If GPU version is built, and multiple GPUs are available, set the ID here
        # params["num_gpu_start"] = 0
        params["disable_blending"] = self.config.disable_blending
        ...
        try:
            ...
            while True:
                ...
                if len(datum.poseKeypoints.shape)>1:
                    ...
                    if len(bboxes) == 0:
                        for bst in poseKeypoints_data_frame:
                            bbox = box_from_bonestruct(bst)
                            bboxes.append(bbox)
                            self.max_number_pers=len(bbox)
                ...
                map_bone= map_bone_struct_to_bbox(poseKeypoints_data_frame,boxes)
                map_bone_video.append(map_bone)



                for  i in range(len(poseKeypoints_data_frame)):

                    if map_bone[i]==-1 and check_bone_struct(poseKeypoints_data_frame[i]):
                        bbox = box_from_bonestruct(poseKeypoints_data_frame[i])
                        bboxes.append(bbox)
                        self.max_number_pers=len(bbox)
                    ...
                    draw_bone_with_color(poseKeypoints_data_frame[i],cv2,show_result,colors[int(map_bone[i])])
